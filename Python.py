...
# Global settings
VERSION = "5.3.5.0"                 # Updated version with separate iterations and UI changes
GAMEPAD_TEST_ITERATIONS = 400       # Number of test iterations for gamepads
KEYBOARD_TEST_ITERATIONS = 100      # Number of test iterations for keyboards
MOUSE_TEST_ITERATIONS = 100         # Number of test iterations for mice
...
TEST_TYPE_MOUSE_BUTTON = "mouse_button"
TEST_TYPE_HARDWARE = "hardware"
...
def draw_text_in_window(screen, text_lines):
    """Draws centered text inside the Pygame window."""
    pygame.font.init()
    ...
    for i, line in enumerate(text_lines):
        ...
        text_rect = text_surface.get_rect(center=(screen_rect.centerx, start_y + i * font.get_height()))
...
class LatencyTester:
    def __init__(self, serial_port, test_type, iterations, gamepad=None, contact_delay=CONTACT_DELAY):
    ...
    def detect_active_mouse_button(self):
        print("\nClick the mouse button that you want to test in the Pygame window...")
        while True:
            for event in pygame.event.get():
                if event.type == QUIT:
                    pygame.quit()
                ...
                if event.type == MOUSEBUTTONDOWN:
                    ...
                    print(f"Selected mouse button #{self.mouse_button_to_test}")
                    time.sleep(0.5)
                    return True

    def detect_active_key(self):
        print("\nPress the key that you want to test in the Pygame window...")
        while True:
            for event in pygame.event.get():
                if event.type == QUIT:
                    pygame.quit()
                ...
                if event.type == KEYDOWN:
                    ...
                    print(f"Selected key: '{key_name}' (Code: {self.key_to_test})")
                    time.sleep(0.5)
                    return True
    ...
    def is_key_pressed(self):
        ...
        return self.key_to_test is not None and keys[self.key_to_test]
    ...
    def is_mouse_button_pressed(self):
        pygame.event.pump()
        if self.mouse_button_to_test is None:
            return False
        return pygame.mouse.get_pressed()[self.mouse_button_to_test - 1]

    def log_progress(self, latency):
        progress = len(self.latency_results)
        print(f"[{progress / self.test_iterations * 100:3.0f}%] {latency:.2f} ms")
    ...
    def test_loop(self):
        print(f"\nStarting {self.test_iterations} measurements with microsecond precision...\n")
        self.trigger_solenoid()
        while len(self.latency_results) < self.test_iterations:
...
if __name__ == "__main__":
    ...
    if main_choice == 1: # Gamepad
        ...
        iterations = GAMEPAD_TEST_ITERATIONS
    ...
    elif main_choice in [2, 3]: # Keyboard or Mouse
        pygame.init()
        screen = pygame.display.set_mode((640, 480)) # Змінено розмір вікна
        pygame_initialized = True
        
        if main_choice == 2:
            pygame.display.set_caption("Prometheus 82 - Keyboard Test")
            test_type = TEST_TYPE_KEYBOARD
            device_name_for_csv = "Keyboard"
            iterations = KEYBOARD_TEST_ITERATIONS
            text = ["This window must be active for the test.", "Press the key you want to test."]
        else: # main_choice == 3
            pygame.display.set_caption("Prometheus 82 - Mouse Test")
            test_type = TEST_TYPE_MOUSE_BUTTON
            device_name_for_csv = "Mouse"
            iterations = MOUSE_TEST_ITERATIONS
            text = ["This window must be active for the test.", "Click the mouse button you want to test."]
        
        draw_text_in_window(screen, text)
    ...
    try:
        with serial.Serial(port_info.device, 115200, timeout=1) as ser:
            ...
            if avg_latency is None:
                print(f"\n{Fore.RED}Error calibrating Arduino latency. Using default CONTACT_DELAY ({CONTACT_DELAY} ms).{Fore.RESET}")
            else:
                CONTACT_DELAY = avg_latency
                print(f"\nSet CONTACT_DELAY to {CONTACT_DELAY:.3f} ms")

            tester = LatencyTester(ser, test_type, iterations, joystick, CONTACT_DELAY)
            ...
            try:
                if test_type == TEST_TYPE_HARDWARE:
                    ...
                    if test_type == TEST_TYPE_BUTTON:
                        ...
                        tester.detect_active_mouse_button()

                    if screen:
                        screen.fill((0, 0, 0)) # Black background
                        draw_text_in_window(screen, ["Testing in progress..."])
